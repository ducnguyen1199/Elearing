...
const CoursesManagement = (props) => {
  ...
  const renderListCourse = () => {
    return listCourse
      .filter((item) => {
        return (
          item.tenKhoaHoc.toLowerCase().indexOf(state.keyword.toLowerCase()) !==
          -1
        );
      })
      .map((item, index) => {
          ...
          <tr key={index}>
            ...
            <td>
              <div className="cm-creator">
                ...
                <p className="cm-type-creator">
                  {item.nguoiTao.tenLoaiNguoiDung}
                </p>
              </div>
            </td>
            <td>{item.ngayTao}</td>
  ...
  return (
    <Fragment>
      ...
      <section className="course-management">
        <h3 className="title">Quản lý khóa học</h3>
